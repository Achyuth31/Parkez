...
function renderParkedList() {
    ...
    if (parked.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--text-muted); padding: 20px;">No matching vehicles parked in the lot currently.</td></tr>`;
        return;
    }
...
function seedHistoryData(facilityId) {
    ...
    // Seed ~800 departed vehicles for the past 3 days
    for (let i = 0; i < 800; i++) {
...
function renderHistoryList() {
    ...
    // Sort: Left (departed) vehicles first, Parked vehicles at the bottom.
    // Within each group, sort by entryTime descending (timeline of entry).
    records.sort((a, b) => {
        const aLeft = a.exitTime !== null;
        const bLeft = b.exitTime !== null;
    ...
    if (records.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: var(--text-muted); padding: 20px;">No history records found for the past 3 days.</td></tr>`;
        return;
    }
